...
Route::middleware('auth')->prefix('location')->name('location.')->group(callback: function (): void {
    ...
    Route::get(uri: '/edit/{id}', action: [LocationController::class, 'edit'])->name(name: 'edit');
    Route::put(uri: '/edit/{id}', action: [LocationController::class, 'update'])->name(name: 'update');
    Route::delete(uri: '/destroy/{id}', action: [LocationController::class, 'destroy'])->name(name: 'destroy');
});
